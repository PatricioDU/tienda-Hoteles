...
def productos(request, accion, id,):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            producto = form.save()
            messages.success(request, f'El producto "{producto}" ha sido {accion} con éxito')
...
def usuarios(request, accion, id):
    ...
    usuario = User.objects.get(id=id) if id != '0' else None
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            usuario = form_usuario.save(commit=False)
            tipo_usuario = form_usuario.cleaned_data['tipo_usuario']
